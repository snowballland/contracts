...
contract SnowballLandToken is ERC20("SnowballLandV2", "SBTv2"), Ownable {
    uint256 private _cap = 10000000e18;
    uint256 private _totalLock;

    uint256 public startReleaseBlock;
    uint256 public endReleaseBlock;
    uint256 public manualMintLimit = 400000e18;
    ...
    constructor(uint256 _startReleaseBlock, uint256 _endReleaseBlock, address _minter) public {
        ...
        // maunalMint 40k for seeding liquidity
        manualMint(_minter, 40000e18);
    }
    ...
    function canUnlockAmount(address _account) public view returns (uint256) {
        // When block number less than startReleaseBlock, no SBTs can be unlocked
        if (block.number < startReleaseBlock) {
            return 0;
        }
        // When block number more than endReleaseBlock, all locked SBTs can be unlocked
        else if (block.number >= endReleaseBlock) {
            return _locks[_account];
        }
        // When block number is more than startReleaseBlock but less than endReleaseBlock,
        // some SBTs can be released
        else
        {
    ...
    function unlock() public {
        require(_locks[msg.sender] > 0, "no locked SBTs");
    ...
    // @dev move SBTs with its locked funds to another account
    ...
    */
    function delegateBySig(
        ...
        require(signatory != address(0), "SBT::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "SBT::delegateBySig: invalid nonce");
        require(now <= expiry, "SBT::delegateBySig: signature expired");
    ...
    */
    function getPriorVotes(address account, uint256 blockNumber) external view returns (uint256) {
        require(blockNumber < block.number, "SBT::getPriorVotes: not yet determined");
    ...
    function _delegate(address delegator, address delegatee) internal {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying SBTs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(block.number, "SBT::_writeCheckpoint: block number exceeds 32 bits");
